...
def create_section():
    ...
    with TaskGroup("inside_section_2") as inside_section_2:
        inside_2_tasks = [EmptyOperator(task_id=f'task-{i + 1}') for i in range(3)]
        logger.info("Created inside_section_2 tasks: %s", inside_2_tasks)
...
with DAG(dag_id="task_group_dag", start_date=days_ago(2), schedule_interval='@daily', default_args=default_args,catchup=False) as dag:
